...
<template>
  <form @submit.prevent="">
    <div class="mb-6">
      <InputGeneral class="mb-5" v-model="gtk.nama" :error="gtk.errors.nama" @clear-error="clearError"
        title="Nama GTK" name="nama" :required="true" />
    ...
    <div class="inline-flex rounded-md shadow-sm" role="group">
      <button type="reset" :disabled="gtk.processing" @click="resetForm"
        :class="{ 'cursor-not-allowed': gtk.processing }"
      ...
      <button :disabled="gtk.processing" :class="{ 'cursor-not-allowed': gtk.processing }"
        @click="$emit('submit')"
